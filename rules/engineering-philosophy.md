...
- **Tests without CI are technical debt.** If a project has `jest.config.js`, `vitest.config.ts` or `playwright.config.ts` but no `.github/workflows/` running them, wiring that workflow comes before the next feature, not after. A test suite that only runs on someone's machine isn't a safety net — it's a false sense of one. See the `ci-cd-and-automation` skill for the concrete setup.
